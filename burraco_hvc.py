...
def interactive_choose_discard( player, pile ) :
...
def interactive_new_group( player ) :
   ...
   while answ != 'd' :
      ...
      if len(group_cards) > 0 :
         print('  new group cards: ', group_cards )
      ...
      if answ != 'd' :
         ...
         if intansw < 1 or intansw > n_choices :
            print(' out of range %d.  try again.' % intansw )
   ...
   still_in_hand = True
   new_group = group( group_cards, is_a_run, still_in_hand )

   print('  new group : ', end='' )
   new_group.print_group2()

   player.group_list.append( new_group )
...
def print_hand4( hand, groups, wildcard_identity ) :
   ...
   for s in range(1,5) :
      ...
      if s == 3 : suit_char = '(h)'
      if s == 4 : suit_char = '(s)'
      print(' suit %d %s : ' % (s, suit_char), end='')
      prev_card = -1
      for c in range(1,15) :
         scih = suit_card_in_hand( c, hand, s )
         if scih > 0 :
...
def key_to_string( key ) :
   ...
   card_str = s_str + c_str

   if s == 0 : card_str = card_str + ' *'
   if c == 2 : card_str = card_str + ' #'
   if d == 0 : card_str = card_str + ' dj'
   if d == 3 : card_str = card_str + ' d2'
...
if __name__ == '__main__':
   ...
   #-------
      ...
      player1.hand.extend( turn_cards )
      player1.hand.sort()
